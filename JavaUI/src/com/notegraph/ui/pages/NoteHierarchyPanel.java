...
public class NoteHierarchyPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        orgBtn = createToggleBtn("Chart", new VectorIcon(VectorIcon.Type.CHART, 14, ColorScheme.PRIMARY_BLUE), true);
        treeBtn = createToggleBtn("Tree", new VectorIcon(VectorIcon.Type.LIST_BULLET, 14, ColorScheme.TEXT_SECONDARY), false);
        JButton graphBtn = createToggleBtn("Graph", new VectorIcon(VectorIcon.Type.NAV_KNOWLEDGE_GRAPH, 14, ColorScheme.TEXT_SECONDARY), false);
        ...
        JButton filterBtn = new JButton("Filter by Tag", new VectorIcon(VectorIcon.Type.FILTER, 14, ColorScheme.TEXT_PRIMARY));
        ...
        JLabel closeBtn = new JLabel(new VectorIcon(VectorIcon.Type.CLOSE, 16, ColorScheme.TEXT_SECONDARY));
    ...
    class OrgChartCanvas extends JPanel {
        ...
        private void drawRootNode(Graphics2D g2, int x, int y, int w, int h, String text) {
            ...
            // Icon + text
            VectorIcon folderIcon = new VectorIcon(VectorIcon.Type.FOLDER, 16, ColorScheme.PRIMARY_BLUE);
            
            g2.setColor(ColorScheme.PRIMARY_BLUE);
            g2.setFont(new Font(ColorScheme.FONT_SEMIBOLD.getFamily(), Font.BOLD, 16));
            FontMetrics fm = g2.getFontMetrics();
            if (fm.stringWidth(text) > w - 40) text = text.substring(0, Math.max(0, 18)) + "...";
            int tw = fm.stringWidth(text);
            int startX = x + (w - tw - 20) / 2;
            folderIcon.paintIcon(OrgChartCanvas.this, g2, startX, y + (h - 16) / 2);
            g2.drawString(text, startX + 22, y + (h + fm.getAscent() - fm.getDescent()) / 2);
        }
        ...
        private void drawTagNode(Graphics2D g2, int x, int y, int w, int h, String tagTitle, int count, Color color) {
            ...
            FontMetrics fm = g2.getFontMetrics();
            String headerText = "#" + tagTitle;
            int tw = fm.stringWidth(headerText);
            int startX = x + (w - tw - 16) / 2;
            VectorIcon tagIcon = new VectorIcon(VectorIcon.Type.TAG, 12, Color.WHITE);
            tagIcon.paintIcon(OrgChartCanvas.this, g2, startX, y + (headerH - 12) / 2);
            g2.drawString(headerText, startX + 16, y + (headerH + fm.getAscent() - fm.getDescent()) / 2);
            // Body text
            g2.setColor(ColorScheme.TEXT_SECONDARY);
        ...
        private void drawNoteNode(Graphics2D g2, int x, int y, int w, int h, String title, Color color) {
            ...
            // Icon + Title
            VectorIcon bookIcon = new VectorIcon(VectorIcon.Type.BOOK, 14, color);
            bookIcon.paintIcon(OrgChartCanvas.this, g2, x + 12, y + (h - 14) / 2);
    ...
    private void buildTreeContent(JPanel treePanel) {
        ...
        for (int i = 0; i < branches.size(); i++) {
            ...
            JLabel dot = new JLabel(new VectorIcon(VectorIcon.Type.DOT, 10, getColorMain(color)));
            ...
            JLabel chevron = new JLabel(new VectorIcon(isExpanded ? VectorIcon.Type.CHEVRON_UP : VectorIcon.Type.CHEVRON_DOWN, 12, ColorScheme.TEXT_TERTIARY));
            ...
            if (isExpanded && branch.leaves != null) {
                ...
                for (ApiModels.GraphLeaf leaf : branch.leaves) {
                    ...
                    JLabel fileIcon = new JLabel(new VectorIcon(VectorIcon.Type.BOOK, 14, ColorScheme.TEXT_SECONDARY));
    ...
    private void updateDetailsSidebar() {
        ...
        JLabel tagIcon = new JLabel(new VectorIcon(VectorIcon.Type.TAG, 28, Color.WHITE));
        ...
        if (selectedBranch.leaves != null) {
            for (ApiModels.GraphLeaf leaf : selectedBranch.leaves) {
                ...
                JLabel icon = new JLabel(new VectorIcon(VectorIcon.Type.BOOK, 14, ColorScheme.TEXT_PRIMARY));
    ...
    // ═══════════════════════════════════════════════
    private JButton createToggleBtn(String text, Icon icon, boolean active) {
    ...
    private void switchView(String mode) {
        ...
        if (orgBtn.getIcon() instanceof VectorIcon vi) orgBtn.setIcon(new VectorIcon(VectorIcon.Type.CHART, 14, mode.equals("org") ? ColorScheme.PRIMARY_BLUE : ColorScheme.TEXT_SECONDARY));
        ...
        if (treeBtn.getIcon() instanceof VectorIcon vi) treeBtn.setIcon(new VectorIcon(VectorIcon.Type.LIST_BULLET, 14, mode.equals("tree") ? ColorScheme.PRIMARY_BLUE : ColorScheme.TEXT_SECONDARY));
