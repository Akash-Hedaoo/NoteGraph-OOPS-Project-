...
public class RevisionPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        JButton generateBtn = createPrimaryButton("Generate AI Plan", new VectorIcon(VectorIcon.Type.SPARKLE, 14, Color.WHITE));
    ...
    private void generatePlan() {
        ...
        statusLabel.setIcon(new VectorIcon(VectorIcon.Type.CLOCK, 14, ColorScheme.PRIMARY_BLUE));
        statusLabel.setForeground(ColorScheme.PRIMARY_BLUE);
        new SwingWorker<List<ApiModels.RevisionPlanItem>, Void>() {
            ...
            protected void done() {
                try {
                    ...
                    statusLabel.setIcon(new VectorIcon(VectorIcon.Type.CHECK, 14, ColorScheme.SUCCESS_TEXT));
                    statusLabel.setForeground(ColorScheme.SUCCESS_TEXT);
                    rebuildPlanGrid();
                } catch (Exception e) {
                    statusLabel.setText(e.getMessage());
                    statusLabel.setIcon(new VectorIcon(VectorIcon.Type.CROSS, 14, ColorScheme.ERROR_TEXT));
    ...
    private RoundedPanel createPlanCard(ApiModels.RevisionPlanItem item) {
        ...
        // Schedule button
        JButton scheduleBtn = createPrimaryButton("Schedule Revision", null);
    ...
    private RoundedPanel createScheduledRow(ApiModels.RevisionScheduleDto rev) {
        ...
        // Status icon
        VectorIcon.Type iconType = switch (rev.status != null ? rev.status : "") {
            case "SENT" -> VectorIcon.Type.MAIL;
            case "COMPLETED" -> VectorIcon.Type.CHECK;
            case "SKIPPED" -> VectorIcon.Type.CHEVRON_RIGHT;
            default -> VectorIcon.Type.CLOCK;
        };
        JLabel iconLbl = new JLabel(new VectorIcon(iconType, 18, ColorScheme.TEXT_SECONDARY));
        ...
        if ("PENDING".equals(rev.status) || "SENT".equals(rev.status)) {
            ...
            JButton completeBtn = createSmallButton("Done", new VectorIcon(VectorIcon.Type.CHECK, 12, ColorScheme.SUCCESS_TEXT), ColorScheme.SUCCESS_TEXT);
            completeBtn.addActionListener(e -> updateStatus(rev.id, "COMPLETED"));
            JButton skipBtn = createSmallButton("Skip", new VectorIcon(VectorIcon.Type.CHEVRON_RIGHT, 12, ColorScheme.TEXT_TERTIARY), ColorScheme.TEXT_TERTIARY);
            skipBtn.addActionListener(e -> updateStatus(rev.id, "SKIPPED"));
            JButton deleteBtn = createSmallButton("", new VectorIcon(VectorIcon.Type.CROSS, 12, ColorScheme.ERROR_TEXT), ColorScheme.ERROR_TEXT);
    ...
    private JButton createSmallButton(String text, Icon icon, Color fg) {
        JButton btn = new JButton(text);
        if (icon != null) btn.setIcon(icon);
